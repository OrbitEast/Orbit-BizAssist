(()=>{
  ...
  const open=async()=>{
    ...
    const renderLines=()=>{linesEl.innerHTML=lines.map((l,i)=>`<div class="invoice-line" data-line="${i}"><div class="line-main"><div class="line-item-summary"><strong>${esc(l.name||'Item not selected')}</strong><span>${esc(l.sku||l.unit||'')}</span></div><div class="line-actions"><button type="button" class="line-edit" data-edit="${i}">Edit</button><button type="button" class="line-remove" data-remove="${i}" ${lines.length===1?'disabled':''}>Remove</button></div></div><div class="line-fields compact"><span><small>Qty</small><b>${l.quantity}</b></span><span><small>Price</small><b>${money(l.unit_price)}</b></span><span><small>Tax</small><b>${Number(l.tax_rate||0)}%</b></span><strong class="line-total">${money(l.quantity*l.unit_price*(1+l.tax_rate/100))}</strong></div></div>`).join('');
