(()=>{
  ...
  const open=async()=>{
    ...
    document.body.appendChild(modal); document.body.classList.add('invoice-ux-lock');
    ...
    const itemPopup=(index=null)=>{
      ...
      pop.innerHTML=`<div class="invoice-item-dialog" role="dialog" aria-modal="true" aria-label="${editing?'Edit':'Add'} invoice item">
        <div class="item-popup-head"><div><span class="invoice-eyebrow">INVOICE ITEM</span><h3>${editing?'Edit item':'Add item'}</h3><p>Fill the item details here — no long page scrolling.</p></div><button type="button" class="invoice-close item-popup-close">×</button></div>
      ...
      pop.querySelector('#popup-save').onclick=()=>{
        ...
        const next={item_id:x.id,name:x.name,sku:x.sku||'',unit:u.value.trim()||x.unit||'pcs',quantity:Math.max(.01,Number(q.value)||0),unit_price:Math.max(0,Number(p.value)||0),tax_rate:Math.max(0,Number(t.value)||0)};
        if(editing)lines[index]=next; else lines.push(next);
        close();renderLines();renderPreview();
      };
      setTimeout(()=>sel.focus(),20);
    };
    const renderLines=()=>{linesEl.innerHTML=lines.length?lines.map((l,i)=>`<div class="invoice-line" data-line="${i}"><div class="line-main"><div class="line-item-summary"><strong>${esc(l.name||'Item not selected')}</strong><span>${esc(l.sku||l.unit||'')}</span></div><div class="line-actions"><button type="button" class="line-edit" data-edit="${i}">Edit</button><button type="button" class="line-remove" data-remove="${i}">Remove</button></div></div><div class="line-fields compact"><span><small>Qty</small><b>${l.quantity}</b></span><span><small>Price</small><b>${money(l.unit_price)}</b></span><span><small>Tax</small><b>${Number(l.tax_rate||0)}%</b></span><strong class="line-total">${money(l.quantity*l.unit_price*(1+l.tax_rate/100))}</strong></div></div>`).join(''):'<div class="invoice-lines-empty">No items added yet. Click <strong>+ Add item</strong> to add a line.</div>';
      linesEl.querySelectorAll('[data-edit]').forEach(b=>b.onclick=()=>itemPopup(Number(b.dataset.edit)));
  ...
  const intercept=e=>{const b=e.target.closest?.('[data-action]');if(!b)return;const action=b.dataset.action;const crumb=(document.querySelector('.workspace-breadcrumb')?.textContent||'').toLowerCase();const invoicePage=crumb.includes('invoices');if(!invoicePage&&!['new-invoice','create-invoice'].includes(action))return;if(action==='quick-add'||action==='new-invoice'||action==='create-invoice'){e.preventDefault();e.stopImmediatePropagation();open()}};
  document.addEventListener('click',intercept,true);
  const watch=()=>{if(mounted)return;if(!/invoice/i.test(document.body.innerText||''))return;if(document.querySelector('[data-action="quick-add"],[data-action="new-invoice"],[data-action="create-invoice"]'))mounted=true};
