/* OrbitBiz — invoice detail, payment entry and lifecycle controls. */
(() => {
  "use strict";
  ...
  const derivedStatus = (invoice) => {
    const total=Number(invoice.total||0), paid=Number(invoice.amount_paid||0), stored=invoice.status||"draft";
    if(stored === "cancelled") return "cancelled";
    if(total>0 && paid>=total) return "paid";
    if(paid>0) return "partial";
    if(invoice.due_date && invoice.due_date < today() && stored !== "draft") return "overdue";
    return stored;
  };
  ...
  w.invoiceDetailView=data=>{
    ...
    const total=Number(invoice.total||0),paid=Number(invoice.amount_paid||0),balance=Math.max(0,total-paid),status=derivedStatus(invoice);
    const statusTone=status==="paid"?"done":status==="partial"?"blue":status==="overdue"?"pink":status==="cancelled"?"open":"yellow";
    ...
    ${status!=="cancelled"?`<div class="ob-invoice-lifecycle"><span>STATUS</span>${["draft","sent","overdue"].map(s=>`<button class="ob-lifecycle-step ${status===s?"active":""}" data-action="invoice-status" data-status="${s}" ${status==="paid"||status==="partial"?"disabled":""}>${esc(s)}</button>`).join("")}<span class="ob-lifecycle-arrow">→</span><button class="ob-lifecycle-step ${status==="cancelled"?"active":""}" data-action="invoice-status" data-status="cancelled">Cancel</button></div>`:""}
  ...
  w.recordInvoicePayment=(state,data)=>{
    ...
    form.addEventListener("submit",async event=>{
      event.preventDefault(); const raw=Object.fromEntries(new FormData(form).entries()),amount=Number(raw.amount||0);
      ...
      try{const result=await w.db().rpc("record_invoice_payment",{p_business_id:state.businessId,p_invoice_id:invoice.id,p_payment_date:raw.payment_date,p_amount:amount,p_method:raw.method,p_reference:raw.reference||null,p_notes:raw.notes||null});if(result.error)throw result.error;node.remove();w.toast("Payment recorded");await state.renderPage();}catch(error){w.toast(error.message||"Could not record payment");button.disabled=false;button.innerHTML=`Save payment ${w.icon("arrow")}`;}
    });
  };

  w.updateInvoiceStatus=async(state,id,status)=>{
    const result=await w.db().rpc("update_invoice_status",{p_invoice_id:id,p_status:status});
    if(result.error) throw result.error;
    w.toast(`Invoice marked ${status}`); await state.renderPage();
  };
})();