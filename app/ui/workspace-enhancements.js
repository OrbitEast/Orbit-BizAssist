...
(()=>{
  ...
  const refresh=()=>{window.dispatchEvent(new Event("orbitbiz:workspace-refresh"));document.querySelector("#workspace-content")?.closest("main")?.querySelector("h1")&&location.reload()};
  ...
  function openPayment(invoices){
    ...
    const invoice=x.querySelector('[name="invoice_id"]'),amount=x.querySelector('[name="amount"]');invoice.onchange=()=>{const o=invoice.selectedOptions[0];amount.value=o?.dataset.due||""};
    x.querySelector("form").onsubmit=async e=>{e.preventDefault();const f=new FormData(e.target),invoiceId=String(f.get("invoice_id")||"");const inv=invoices.find(i=>i.id===invoiceId);const value=Number(f.get("amount")||0);if(!inv||value<=0){toast("Select an invoice and enter a valid amount");return}const due=Math.max(0,Number(inv.total)-Number(inv.amount_paid));if(value>due+0.005){toast(`Payment cannot exceed the outstanding ${money(due)}`);return}const p={business_id:businessId(),invoice_id:invoiceId,customer_id:null,payment_date:f.get("payment_date")||today(),amount:value,method:String(f.get("method")||"bank_transfer"),reference:String(f.get("reference")||"").trim()||null,notes:String(f.get("notes")||"").trim()||null};const ins=await db().from("payments").insert(p);if(ins.error){toast(ins.error.message);return}const paid=Number(inv.amount_paid)+value;const status=paid>=Number(inv.total)-0.005?"paid":"partial";const up=await db().from("invoices").update({amount_paid:paid,status}).eq("id",invoiceId).eq("business_id",businessId());if(up.error){toast(`Payment saved, but invoice balance could not update: ${up.error.message}`);x.remove();return}x.remove();toast("Payment recorded successfully");await paymentsPage()};
  }
  ...
  function openExpense(){
    ...
    x.querySelector("form").onsubmit=async e=>{e.preventDefault();const f=new FormData(e.target);const r=await db().from("expenses").insert({business_id:businessId(),expense_date:f.get("expense_date")||today(),category:String(f.get("category")||"").trim(),description:String(f.get("description")||"").trim()||null,amount:Number(f.get("amount")||0),tax_amount:Number(f.get("tax_amount")||0),payment_method:String(f.get("payment_method")||"")||null,reference:String(f.get("reference")||"").trim()||null});if(r.error){toast(r.error.message);return}x.remove();toast("Expense added successfully");await expensesPage()};
  }
  ...
  function openAccount(){
    ...
    x.querySelector("form").onsubmit=async e=>{e.preventDefault();const f=new FormData(e.target);const name=String(f.get("name")||"").trim();if(!name){toast("Account name is required");return}const r=await db().from("payment_accounts").insert({business_id:businessId(),name,account_type:String(f.get("account_type")||"bank"),opening_balance:Number(f.get("opening_balance")||0),current_balance:Number(f.get("current_balance")||0),is_active:true});if(r.error){toast(r.error.message);return}x.remove();toast("Payment account added successfully");await accountsPage()};
  }

  async function postNewInvoiceStock(){
    const key=`orbitbiz.stock.posted.${businessId()}`;const posted=new Set(JSON.parse(localStorage.getItem(key)||"[]"));
    ...
    for(const inv of invoices||[]){if(posted.has(inv.id))continue;const {data:lines}=await db().from("invoice_items").select("item_id,quantity").eq("invoice_id",inv.id);if(!lines?.length){continue}for(const line of lines){if(!line.item_id)continue;const exists=await db().from("stock_movements").select("id").eq("business_id",businessId()).eq("reference_id",inv.id).eq("item_id",line.item_id).eq("movement_type","sale").limit(1);if(exists.data?.length)continue;await db().from("stock_movements").insert({business_id:businessId(),item_id:line.item_id,quantity:-Math.abs(Number(line.quantity||0)),movement_type:"sale",reference_id:inv.id,notes:`Invoice ${inv.invoice_number}`})}posted.add(inv.id)}
    localStorage.setItem(key,JSON.stringify([...posted].slice(-100)));
  }

  async function enhance(){
    if(!businessId()||!document.querySelector("#workspace-content"))return;
    const heading=document.querySelector("#workspace-content h2")?.textContent?.trim().toLowerCase();
    try{
      if(heading==="receipts")await paymentsPage();
      else if(heading==="purchases")await expensesPage();
      else if(heading==="payment accounts")await accountsPage();
  ...
  window.addEventListener("orbitbiz:auth-ready",()=>setTimeout(enhance,120));
  window.addEventListener("auth:ready",()=>setTimeout(enhance,120));
