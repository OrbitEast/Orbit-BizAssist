...
(() => {
  ...
  function getUser() { return window.AppState?.get?.()?.user || null; }
  function getBusiness() {
    const current = window.AppState?.get?.();
    if (!current) return null;
    return current.businesses?.find(business => business.id === current.activeBusiness) || current.businesses?.[0] || null;
  }
  function userName() {
    const user = getUser();
  ...
  function navigationItem(item, current) {
    ...
    return `<button class="nav-item ${active ? "active" : ""}" type="button" data-route="${item.id}" aria-current="${active ? "page" : "false"}"><span aria-hidden="true">${item.icon}</span><strong>${item.label}</strong></button>`;
  }

  function sidebar() {
    const current = activeRoute();
    const business = getBusiness();
    return `<aside class="sidebar"><div class="sidebar-brand"><span class="brand-mark" aria-hidden="true">O</span><span>Orbit BizAssist</span></div><div class="business-switch"><small>BUSINESS</small><strong>${escapeHTML(business?.name || "My Business")}</strong></div><nav class="sidebar-nav" aria-label="Main navigation">${NAV_ITEMS.map(item => navigationItem(item, current)).join("")}<div class="nav-divider"></div>${MORE_ITEMS.map(item => navigationItem(item, current)).join("")}</nav><div class="sidebar-bottom"><button class="nav-item" type="button" data-action="signout"><span aria-hidden="true">↪</span><strong>Sign out</strong></button></div></aside>`;
  }
  ...
  function topbar() {
    ...
    const name = userName();
    return `<header class="topbar"><div><span class="eyebrow">${escapeHTML(getBusiness()?.name || "Workspace")}</span><h1>${pageTitle(route)}</h1></div><div class="topbar-actions"><button class="icon-btn mobile-menu-button" type="button" data-action="mobile-menu" aria-label="Open navigation" title="Menu">☰</button><button class="icon-btn" type="button" data-action="notifications" aria-label="Notifications" title="Notifications">♢</button><div class="user-avatar" title="${escapeHTML(name)}" aria-label="${escapeHTML(name)}">${escapeHTML(initials(name))}</div></div></header>`;
  }
  ...
  function render() {
    ...
    const onboarding = window.AppState?.get?.()?.onboarding;
    ...
    if (!onboarding?.completed) {
      ...
      root.setAttribute("aria-busy", "false");
      setTimeout(() => window.AppOnboarding?.bind?.(), 0);
      return;
    }
  ...
  function handleClick(event) {
    ...
    if (routeTarget) {
      ...
      if (window.AppRouter?.go) window.AppRouter.go(routeTarget.dataset.route);
      return;
    }
    ...
    if (action === "notifications") { if (typeof toast === "function") toast("No new notifications."); return; }
    if (action === "mobile-menu") toggleMobileNavigation();
  }
  ...
  window.addEventListener("hashchange", () => { closeMobileNavigation(); render(); });

  window.AppUI = Object.freeze({ render, appShell, sidebar, topbar, routeView, closeMobileNavigation });
